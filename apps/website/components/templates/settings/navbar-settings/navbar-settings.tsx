...
import { AccountCircle, ManageAccounts, ChevronRight, Hub, Notifications } from '@mui/icons-material';
import { Box, Divider, ListItemButton, Stack, SvgIconTypeMap, Theme } from '@mui/material';
          ...
          <Link key={menuItem.key} passHref href={menuItem.href}>
            <ListItemButton
              ...
              sx={(theme) => ({
                ...
                background: menuItem.href === router.pathname ? '#9A53FF16' : null,
                '&:hover': {
                  background: menuItem.href === router.pathname ? '#9A53FF16' : null,
                },
            ...
            >
              ...
              <Box sx={{ flexGrow: 1 }}>
                ...
                <Typography variant='body2' sx={(theme) => ({ color: theme.palette.grey[500] })}>{menuItem.description}</Typography>
              </Box>
              <ChevronRight color="disabled" sx={(theme) => ({ flexGrow: 0, color: theme.palette.grey[400] })} />
            </ListItemButton>
          </Link>
          {i !== menuSettings.length - 1 && (
            <Divider />
          )}
          </>
        ))}
